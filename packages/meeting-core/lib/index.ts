...
export const MEETING_CONSTANTS = {
  ...
  TEAMS_PATTERNS: [/.*\.teams\.microsoft\.com\/.*\/conversations\/.*/, /.*\.teams\.microsoft\.com\/.*\/meetings\/.*/],

  // File type patterns for meeting recordings
  VIDEO_PATTERNS: [/\.(mp4|webm|avi|mov|wmv|flv|mkv)$/i],

  AUDIO_PATTERNS: [/\.(mp3|wav|ogg|aac|m4a|flac|wma)$/i],
...
export const MeetingUtils = {
  ...
  isMeetingUrl(url: string): boolean {
    const patterns = [...MEETING_CONSTANTS.SHAREPOINT_PATTERNS, ...MEETING_CONSTANTS.TEAMS_PATTERNS];
  ...
  isSupportedMediaFile(filename: string): boolean {
    return [...MEETING_CONSTANTS.VIDEO_PATTERNS, ...MEETING_CONSTANTS.AUDIO_PATTERNS].some(pattern =>
